...
import { Users, Heart, BookOpen, Home, TrendingUp, FileText } from "lucide-react";
...
const TICK_STYLE   = { fontSize: 11, fontFamily: "Inter", fill: "hsl(210,10%,45%)" };
const TOOLTIP_STYLE = {
  borderRadius: "12px",
...
function SectionHeader({ title, subtitle }: { title: string; subtitle?: string }) {
  return (
    <div className="mb-5">
      ...
      {subtitle && <p className="font-body text-sm text-muted-foreground mt-0.5">{subtitle}</p>}
    </div>
  );
}
...
function ReportsPage() {
  ...
  const totalResidents = SAFEHOUSE_PERFORMANCE.reduce((s, sh) => s + sh.admitted, 0);
  const totalReintegrated = REINTEGRATION_OUTCOMES.reduce((s, o) => s + o.value, 0);
  const totalServices = SERVICES_BY_QUARTER.reduce((s, q) => s + q.caring + q.healing + q.teaching, 0);
  ...
  return (
    <div className="min-h-screen bg-background font-body">
      ...
      <main className="ml-64 p-8">
        ...
        <div className="flex items-start justify-between mb-8">
          <div>
            ...
            <p className="font-body text-base text-muted-foreground mt-1">
              Aggregated insights aligned with the Annual Accomplishment Report format.
            </p>
          </div>
        ...
        <div className="bg-[#FDFBF7] border-t-4 border-t-yellow-500 rounded-2xl p-6 mb-8 shadow-sm">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-card rounded-2xl border border-border p-5">
              ...
              <div className="font-heading text-3xl font-bold text-foreground">{totalResidents}</div>
              <div className="font-body text-sm font-semibold text-foreground mt-0.5">Total Beneficiaries</div>
              <div className="font-body text-xs text-muted-foreground">Residents admitted this year</div>
            </div>
            ...
            <div className="bg-card rounded-2xl border border-border p-5">
              ...
              <div className="font-heading text-3xl font-bold text-foreground">{aarCaring}</div>
              <div className="font-body text-sm font-semibold text-foreground mt-0.5">Caring</div>
              <div className="font-body text-xs text-muted-foreground">Shelter, food & safety service days</div>
            </div>
            ...
            <div className="bg-card rounded-2xl border border-border p-5">
              ...
              <div className="font-heading text-3xl font-bold text-foreground">{aarHealing}</div>
              <div className="font-body text-sm font-semibold text-foreground mt-0.5">Healing</div>
              <div className="font-body text-xs text-muted-foreground">Counseling & psychosocial sessions</div>
            </div>
            ...
            <div className="bg-card rounded-2xl border border-border p-5">
              ...
              <div className="font-heading text-3xl font-bold text-foreground">{aarTeaching}</div>
              <div className="font-body text-sm font-semibold text-foreground mt-0.5">Teaching</div>
              <div className="font-body text-xs text-muted-foreground">Livelihood & education trainings</div>
            </div>
          </div>
          ...
          <div className="mt-4 grid grid-cols-3 gap-3 text-center">
            {[
              { label: "Total Services Rendered", value: totalServices, color: "text-foreground" },
              { label: "Successfully Reintegrated", value: totalReintegrated, color: "text-primary" },
              { label: "Total Contributions Received", value: `₱${totalDonations.toLocaleString("en-PH")}`, color: "text-yellow-700" },
            ].map((s) => (
              <div key={s.label} className="bg-background rounded-xl border border-border py-3 px-4">
        ...
        {/* ── Donation Trends ──────────────────────────────────────────────── */}
        <div className="mb-8">
          <SectionHeader title="Donation Trends" subtitle="Monthly monetary contributions across all campaigns" />
          <div className="bg-card rounded-2xl border border-border shadow-sm p-6">
            <ResponsiveContainer width="100%" height={260}>
              <AreaChart data={DONATION_TREND}>
                ...
                <YAxis tick={TICK_STYLE} axisLine={false} tickLine={false} tickFormatter={formatPHP} width={56} />
                <Tooltip
                  formatter={(v) => [`₱${Number(v).toLocaleString("en-PH")}`, "Amount"]}
                  contentStyle={TOOLTIP_STYLE}
                />
                <Area type="monotone" dataKey="amount" stroke={C_PRIMARY} strokeWidth={2} fill="url(#donGrad)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6 mb-8">
          ...
          <div>
            <SectionHeader title="Safehouse Performance" subtitle="Residents admitted, active, and graduated per safehouse" />
            <div className="bg-card rounded-2xl border border-border shadow-sm p-6">
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={SAFEHOUSE_PERFORMANCE} barCategoryGap="30%">
                  ...
                  />
                  <Bar dataKey="admitted"  name="Admitted"  fill={C_BLUE}    radius={[4, 4, 0, 0]} />
                  <Bar dataKey="active"    name="Active"    fill={C_PRIMARY}  radius={[4, 4, 0, 0]} />
                  <Bar dataKey="graduated" name="Graduated" fill={C_GREEN}    radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div>
            <SectionHeader title="Reintegration Outcomes" subtitle={`${totalReintegrated} cases completed the reintegration process`} />
            <div className="bg-card rounded-2xl border border-border shadow-sm p-6">
              <ResponsiveContainer width="100%" height={240}>
                <PieChart>
                  <Pie
        ...
        <div className="grid lg:grid-cols-2 gap-6 mb-8">
          {/* Services by Quarter */}
          <div>
            <SectionHeader title="Services by Quarter" subtitle="Caring, Healing, and Teaching service counts per quarter" />
            <div className="bg-card rounded-2xl border border-border shadow-sm p-6">
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={SERVICES_BY_QUARTER} barCategoryGap="28%">
                  ...
                  <Bar dataKey="caring"   name="Caring"   fill={C_YELLOW}  radius={[4, 4, 0, 0]} />
                  <Bar dataKey="healing"  name="Healing"  fill={C_PURPLE}  radius={[4, 4, 0, 0]} />
                  <Bar dataKey="teaching" name="Teaching" fill={C_BLUE}    radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div>
            <SectionHeader title="Beneficiaries by Case Category" subtitle="Distribution of admitted residents by case type" />
            <div className="bg-card rounded-2xl border border-border shadow-sm p-6">
              <ResponsiveContainer width="100%" height={240}>
                <BarChart
                ...
                >
                  ...
                  />
                  <Tooltip contentStyle={TOOLTIP_STYLE} />
                  <Bar dataKey="count" name="Residents" fill={C_PINK} radius={[0, 4, 4, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="mb-8">
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm p-6">
              ...
              {OUTCOME_INDICATORS.map((o) => (
                <div key={o.label}>
                  <div className="flex justify-between items-baseline mb-1.5">
                    <span className="font-body text-sm font-medium text-foreground">{o.label}</span>
                    <span className="font-heading text-sm font-bold text-foreground">{o.pct}%</span>
                  </div>
            ...
            <p className="font-body text-xs text-muted-foreground mt-5">
              Indicators measured at case closure through standardized welfare assessment forms. Target threshold: 70%.
            </p>
          </div>
        </div>
        ...
        <div className="bg-muted/50 rounded-2xl border border-border p-5 flex items-start gap-3">
          ...
          <div>
            <p className="font-body text-sm font-medium text-foreground mb-0.5">Mock Data — Backend Not Yet Connected</p>
            <p className="font-body text-xs text-muted-foreground leading-relaxed">
              All figures shown are representative sample data. Wire each chart's data source to the corresponding
              C# API endpoint. The year selector state (<code className="font-mono bg-muted px-1 rounded">reportYear</code>)
              should be passed as a query parameter to filter results by reporting period.
            </p>
          </div>
        </div>
